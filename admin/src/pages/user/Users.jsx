...
export default function Users() {
      ...
      renderCell: (params) => (
        ...
        >
          ...
          >
            <Typography
              variant="body2"
              sx={{ fontWeight: 800, fontSize: "0.975rem", color: "#ecfeff" }}
            >
              {params.row.name}
            </Typography>
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          <Tooltip title="Edit User">
            <IconButton
              ...
              sx={{
                color: "#3dd6c6",
                "&:hover": { bgcolor: "rgba(61, 214, 198, 0.1)" },
              }}
            >
              <FaEdit size={16} />
            </IconButton>
  ...
  useClickOutside(elemRef, () => setOpen(false));

  return (
    <ThemeProvider theme={darkTheme}>
      <div className=" min-h-screen" style={{ backgroundColor: "#01090d" }}>
        {/* Header Section */}
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-3 border border-primary/30  p-4  gap-4">
          <div>
            <h1
            ...
            <p style={{ color: "#7fa5a8" }} className="text-sm">
              Monitor and manage access controls across the platform.
            </p>
          </div>
        ...
        {open && <div className="backdrop-blur-sm fixed inset-0"></div>}
        {open && (
          <div
            className="fixed inset-0 flex pt-20 items-center justify-center p-4"
            onClick={() => setOpen(false)}
          >
            <div ref={elemRef} onClick={(e) => e.stopPropagation()}>
              <NewUser />
            </div>
          </div>
        )}

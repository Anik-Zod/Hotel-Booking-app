import { MdClose, MdPeopleOutline, MdVerifiedUser, MdFlashOn, MdCreditCardOff } from 'react-icons/md';
...
export default function Reserve({ setOpen, hotelId , TotalPrice}) {
  ...
  if (isError)
    ...
    return (
    <div className="backdrop-blur-xl bg-white/90 rounded-[32px] shadow-[0_32px_80px_-16px_rgba(0,0,0,0.3)] w-full max-w-2xl mx-auto overflow-hidden border border-white/50 ring-1 ring-black/5">
      ...
      <div className="px-8 max-h-[60vh] overflow-y-auto scroll-smooth custom-scrollbar bg-gradient-to-b from-white to-slate-50/30">
        <div className="divide-y divide-gray-100">
          {filteredData.map((item) => item && (
            <div key={item._id} className="py-8 first:pt-6 last:pb-8 group">
              {/* Room Info */}
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="flex gap-2 pt-1">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 text-[#003B95] rounded-full text-[11px] font-bold uppercase tracking-wider border border-blue-100">
                      <MdPeopleOutline className="text-sm" /> {item.maxPeople} Guests
                    </span>
                  </div>
                </div>

                <div className="text-right shrink-0">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Nightly</p>
                  <div className="flex items-baseline justify-end">
                    <span className="text-2xl font-black text-slate-900 tracking-tighter">${item.price}</span>
                    <span className="text-sm font-bold text-slate-400 ml-1">/nt</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                {item.roomNumbers.map((roomNumber) => {
                  ...
                  return (
                    <label key={roomNumber._id} className="relative cursor-pointer group/chip">
                      <input
                        type="checkbox"
                      ...
                      `}>
                        <span className={`text-sm font-black ${available ? "text-slate-700 peer-checked:text-[#003B95]" : "text-slate-400"}`}>
                          {roomNumber.number}
                        </span>
                        {available && (
                          <span className="text-[8px] font-bold uppercase text-slate-400 peer-checked:text-[#003B95]">Select</span>
                        )}
                      </div>
